  ...
  render() {
    if (!this.props.loading) {
      ...
      if (this.props.path.status === "current") {
        pathProgressBtn = (
          <Link
            to={`/dashboard/classroom/${this.props.path.id}/${
              this.props.lastVideoIndex
            }`}
          >
            <button className="path-progress-btn">Continue</button>
          </Link>
        );
      } else {
        pathProgressBtn = (
          <Link to={`/dashboard/classroom/${this.props.path.id}/0`}>
            <button
        ...
        <div className="path-overview-container">
          <section className="path-info-container">
            ...
            <div className="progress-btn-container">
              ...
              {this.props.status === "none" || this.props.status === "saved"
                ? saveButton
                : undefined}
            </div>
          </section>
          <section className="path-videos-info-container">
            <ul className="path-videos-list">{videos}</ul>
          </section>
        </div>
      );
    }
